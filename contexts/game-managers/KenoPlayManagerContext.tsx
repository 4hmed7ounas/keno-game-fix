...
const KenoPlayManagerContext = createContext<
  KenoPlayManagerContextProps | undefined
>(undefined);

export const KenoPlayManagerProvider = ({
  children,
}: {
  children: ReactNode;
}) => {
  ...
  const fetchNextGame = async () => {
    try {
      ...
      let previousWinningNumbers =
        displayGame?.keno.winningNumbers.length > 0
          ? displayGame.keno.winningNumbers
          : generateUniqueRandomNumbers();
      setPreviousGame({
        ...(displayGame ?? nextGame),
        keno: {
          ...(displayGame ?? nextGame).keno,
          winningNumbers: previousWinningNumbers,
        },
      });
  ...
  const scheduleFetchWinningNumber = (game: IKenoGame) => {
    ...
    let winningNumberFetchTime = new Date(
      game.keno.ticketWillBeDisabledAt,
    ).getTime();
    const timeUntilWinningNumber = winningNumberFetchTime - now;

    if (timeUntilWinningNumber > 0) {
      fetchWInningTimeoutRef.current = setTimeout(
        () => fetchWinningNumbers(),
        timeUntilWinningNumber,
      );
    }
  };
  ...
  useEffect(() => {
    ...
    const updateGameState = () => {
      const now = new Date().getTime();
      const ticketDisabledTime = new Date(
        displayGame.keno.ticketWillBeDisabledAt,
      ).getTime();
      const showWinningNumbersTime = new Date(
        displayGame.keno.winningNumberWillBeShowedAt,
      ).getTime();
  ...
  return (
    <KenoPlayManagerContext.Provider
      value={{
        ...
        currentPlayGameId: displayGame
          ? getLastGameId(displayGame.uniqueId)
          : "",
        previousGameId: previousGame
          ? getLastGameId(previousGame.uniqueId)
          : "",
      }}
    >
      {children}
...
export const useKenoPlayManager = () => {
  ...
  if (context === undefined) {
    throw new Error(
      "useKenoPlayManager must be used within an KenoPlayManagerProvider",
    );
  }
  return context;
};
